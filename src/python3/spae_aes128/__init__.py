...
class SpaeCipher(object):
    ...
    def __init__(self, key, nonce, decrypt, CSPAE):
        ...
        if CSPAE:
            self.__ct = self.cipher_enc(self.xor(key,nonce),key)
            self.__pt = self.xor(self.xor(key,nonce),self.__ct)
            self.__cipher = self.get_cipher(key)
        else:
            self.__ct = self.cipher_enc(key,key)
            self.__pt = self.xor(key,self.__ct)
...
def SPAE_selftest():
    ...
    assert(encrypted00==binascii.unhexlify(b'7da6afe85da1cd2c8b433c2c66c76ef8'))
