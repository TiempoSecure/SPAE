...
#ifndef __SPAE_FLE_H__
#define __SPAE_FLE_H__

#include <limits.h>
#include <stdint.h>

//#if UINT_MAX == UINT32_MAX
//    #define SPAE_FLE_ALIGNEMENT_MASK 0x03
//#endif
//#if UINT_MAX == UINT64_MAX
//    #define SPAE_FLE_ALIGNEMENT_MASK 0x07
//#endif
//TODO: fix this, on x86_64 we get 0x03 and then run time errors...
#define SPAE_FLE_ALIGNEMENT_MASK 0x07

#define SPAE_FLE_IS_ALIGNED(a) (0==(((size_t)a)&SPAE_FLE_ALIGNEMENT_MASK))

#define SPAE_FLE_DBG_EN 0

#if SPAE_FLE_DBG_EN
    #define SPAE_FLE_DBG(a) a;
#else
    #define SPAE_FLE_DBG(a)
#endif
...
#define SPAE_FLE_KEYSIZE 16

#define SPAE_FLE_BLOCKSIZE64 2
...
struct spae_aes128_struct_t {
    uint64_t pt[SPAE_FLE_BLOCKSIZE64];
...
static void spae_aes128_xor(
    ...
    for(unsigned int i=0;i<SPAE_FLE_BLOCKSIZE;i++){
        yb[i] = ab[i] ^ bb[i];
    }
}
...
static mbedtls_aes_context spae_mbedtls_aes_ctx;
 ...
 */
static void aes_enc128_block_le32(
...
static void spae_aes128_enc_core_aligned(
    ...
    buf[1]=ctx->ct[1];
    spae_aes128_xor_64(ctx->ct,ctx->pt,ctx->ct);
    spae_aes128_xor_64(ctx->pt,ib,buf2);
    aes_enc128_block_le32(buf2,buf2);
    spae_aes128_xor_64(buf2,ib,ctx->pt);
    spae_aes128_xor_64(buf2,buf,(uint64_t*)ctx->obuf);
    ctx->obuf+=SPAE_FLE_BLOCKSIZE;
}
...
static void spae_aes128_init(
    ...
    memcpy(k,key,16);
    //aes_enc128_init(k);
    mbedtls_aes_setkey_enc(&spae_mbedtls_aes_ctx, (const uint8_t* const)k,128 );

    aes_enc128_block_le32(k,ctx->ct);
    spae_aes128_xor_64(ctx->ct,k,ctx->pt);

    if(SPAE_FLE_IS_ALIGNED(nonce)) spae_aes128_xor_64(k,(const uint64_t*const)nonce,ctx->kn);
    else spae_aes128_xor(k,nonce,ctx->kn);
    memset(ctx->at,0x00,SPAE_FLE_BLOCKSIZE);
}
...
static void spae_aes128_dec_core(
    ...
    uint8_t buf[SPAE_FLE_BLOCKSIZE];
...
static void spae_compute_tag(
    ...
    if(m){
        ...
        for(unsigned int i=0;i<SPAE_FLE_BLOCKSIZE64;i++){buf[i] = ~buf[i];}
    }
    spae_aes128_xor_64(ctx->at,buf,buf);

    uint64_t padinfo64[SPAE_FLE_BLOCKSIZE64] = {0};
    uint32_t mlen32[2];
    uint32_t alen32[2];
    mlen32[0]=mlen*8;
    mlen32[1]=0;
    ...
    SPAE_FLE_DBG(println_128("PADINFO=",(uint8_t*)padinfo64));
...
void spae_fle_aes128_enc(
    ...
    size_t m = (mlen + SPAE_FLE_BLOCKSIZE - 1) / SPAE_FLE_BLOCKSIZE;
    size_t a = (alen + SPAE_FLE_BLOCKSIZE - 1) / SPAE_FLE_BLOCKSIZE;

    SPAE_FLE_DBG((void)spae_print_state;)
    SPAE_FLE_DBG(println_128("key=",key);)
    SPAE_FLE_DBG(println_128("nonce=",nonce);)
    SPAE_FLE_DBG(printf("mlen=%lu\n",mlen));
    if(m){
        SPAE_FLE_DBG(for(unsigned int i = 0;i<((unsigned int)m)-1;i++){println_128("m=",message+SPAE_FLE_BLOCKSIZE*i);});
        SPAE_FLE_DBG(printf("m=");for(unsigned int i = (m-1)*SPAE_FLE_BLOCKSIZE;i<mlen;i++){printf("%02X",*(message+i));};printf("\n"));
    }
    SPAE_FLE_DBG(printf("alen=%lu\n",alen));
    if(a){
        SPAE_FLE_DBG(for(unsigned int i = 0;i<((unsigned int)a)-1;i++){println_128("a=",ad+SPAE_FLE_BLOCKSIZE*i);});
        SPAE_FLE_DBG(printf("a=");for(unsigned int i = (a-1)*SPAE_FLE_BLOCKSIZE;i<alen;i++){printf("%02X",*(ad+i));};printf("\n"));
    }
    ...
    if(a){
        ...
        uint8_t buf[SPAE_FLE_BLOCKSIZE] = {0};
        unsigned int remaining = alen % SPAE_FLE_BLOCKSIZE;
    ...
    if(m){
        unsigned int remaining = mlen % SPAE_FLE_BLOCKSIZE;
        if((remaining==0) && SPAE_FLE_IS_ALIGNED(in) && SPAE_FLE_IS_ALIGNED(ctx->obuf)){
            for(size_t i = 0; i<m; i++){//process all blocks except last one
                spae_aes128_enc_core_aligned(ctx,(const uint64_t*const)in);
            ...
            if(SPAE_FLE_IS_ALIGNED(in) && SPAE_FLE_IS_ALIGNED(ctx->obuf)){
                ...
                uint64_t buf64[SPAE_FLE_BLOCKSIZE64];
                if(remaining==0) {;//copy always last block into a buffer to support out=in case
                    ...
                    buf64[0] = 0;
                    buf64[1] = 0;
                    memcpy(buf64,in,remaining);
                }
                spae_aes128_enc_core_aligned(ctx,buf64);
            }else{
                for(size_t i = 0; i<m-1; i++){//process all blocks except last one
                    spae_aes128_enc_core(ctx,in);
                    in+=SPAE_FLE_BLOCKSIZE;
                }
                uint64_t buf64[SPAE_FLE_BLOCKSIZE64] = {0};
                uint8_t *buf = (uint8_t *)buf64;
                if(remaining==0) remaining = SPAE_FLE_BLOCKSIZE;//copy always last block into a buffer to support out=in case
                memcpy(buf,in,remaining);
                spae_aes128_enc_core(ctx,(const uint8_t*const)buf64);
            }
        }
    }
    ...
    *clen = ctx->obuf - ciphertext;
    ...
    SPAE_FLE_DBG(printf("clen=%lu\n",*clen));
    SPAE_FLE_DBG(for(unsigned int i = 0;i<(*clen/SPAE_FLE_BLOCKSIZE);i++){println_128("c=",ciphertext+SPAE_FLE_BLOCKSIZE*i);});
    SPAE_FLE_DBG(printf("\n"));
}
...
int spae_fle_aes128_dec(
    ...
    if(clen<SPAE_FLE_BLOCKSIZE) return -1;
    ...
    SPAE_FLE_DBG(printf("clen=%lu\n",clen));
    SPAE_FLE_DBG(assert(0==(clen%SPAE_FLE_BLOCKSIZE)));
    SPAE_FLE_DBG(for(unsigned int i = 0;i<(clen/SPAE_FLE_BLOCKSIZE);i++){println_128("c=",ciphertext+SPAE_FLE_BLOCKSIZE*i);});
    SPAE_FLE_DBG(printf("mlen=%lu\n",mlen));
    SPAE_FLE_DBG(printf("alen=%lu\n",alen));
    if(a){
    ...
    if(memcmp(buf,in,SPAE_FLE_BLOCKSIZE)){
        memset(message,0,mlen);//erase all output
        SPAE_FLE_DBG(printf("tag mismatch!\n"));
        SPAE_FLE_DBG(println_128("computed tag=",buf));
        SPAE_FLE_DBG(println_128("expected tag=",in));
        SPAE_FLE_DBG(printf("\n"));
        return ~SPAE_FLE_PASS;
    }
